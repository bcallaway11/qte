...
(ATT) summarizes the causal impact of a policy by a single number: the mean
...
The Quantile Treatment Effect at level $\tau$ is $QTE(\tau) = Q_Y(1)(\tau) -
Q_Y(0)(\tau)$, where $Q_Y(d)(\tau)$ is the $\tau$-th quantile of the
...
and staggered treatment adoption, see `vignette("did-estimators")` and
...
By default `autoplot()` shows the uniform confidence band (`cband = TRUE`).
Pass `cband = FALSE` for pointwise intervals.

Standard errors are computed via the empirical bootstrap. The `biters`
argument controls the number of iterations (default 100; use 499 or more for
publication). Parallel computation is available via the `cl` argument.
