...
[fig:quantiles]: ![A plot of a standard normal distribution truncated from the left at zero.  The lower dotted line represents the median; the upper dotted line represents the 95th percentile.](figures/cdf-truncated-normal.pdf)

![The figure on the right inverts the same distribution to show the quantiles.  Each dotted line once again corresponds to the median and the 95th percentile.](figures/quantiles-truncated-normal.pdf)
...
Many quantiles are quite familiar.  The 0.5-quantile is the median.  The 0.25- and 0.75-quantiles are the 1st and 3rd quartile.  The 0.01-, 0.02-, 0.03- \ldots are the 1st, 2nd, 3rd, and so on percentiles.  [Figure][fig:quantiles](a) plots the distribution function of random draws from a $N(1,1)$ distribution truncated at 0.  From the distribution function, the $\tau$-th quantile can be found by (i) finding $\tau$ on the y-axis, (ii) finding where a horizontal line from that point intersects the distribution function, and (iii) drawing a vertical line down to the y-axis.  In [Figure][fig:quantiles](a), this procedure is followed to find the median and the 95-th percentile of the distribution.  In [Figure][fig:quantiles](b), the entire distribution from [Figure][fig:quantiles](a) is inverted to obtain all the quantiles.  The dotted lines in [Figure][fig:quantiles](b) correspond to the median and the 95-th percentile.
...
```
The [@callaway-li-2016] method can be invoked with the `panel.qtet` command:
...
The QTET for participating in the job training program is estimated using each of the three methods ddid2, QDiD, CiC, and MDiD next.
...
Finally, plots for all three methods are combined with the plot for the QTET coming from the experimental data
