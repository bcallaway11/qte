...
`qte` is a CRAN R package (v1.4.0) for computing Quantile Treatment Effects
(QTE) and Quantile Treatment Effects on the Treated (QTET).

Identification strategies covered:
1. **Random assignment** — `ci.qte`, `ci.qtet` (Firpo 2007)
2. **Selection on observables** — `DR` (doubly robust)
3. **DiD-based** — `CiC` (Athey-Imbens 2006), `QDiD`, `MDiD`, `ddid2`, `panel.qtet`

Multi-period / staggered treatment: `cic2` + `cic_attgt` (beta, built on `ptetools`).
...
- **`compute.*`** functions are called both for point estimates and inside
...
- `pte` package has been renamed to `ptetools`; deprecation warning fires on load
