# =============================================================================
# Title: Cross-sectional QTE under selection on observables (ci.qte)
# Description: Implements Firpo (2007) propensity-score reweighting estimator
#   for the Quantile Treatment Effect (QTE) under conditional independence.
#   The treated and untreated quantile functions are reweighted by IPW weights.
...
#' @title ci.qte
#'
#' @description Estimates the Quantile Treatment Effect (QTE) under a
#' Conditional Independence Assumption (selection on observables) using the
#' method of Firpo (2007). Propensity score reweighting is used to construct
#' the counterfactual quantile function. Standard errors are computed via the
#' bootstrap.
#'
...
ci.qte <- function(formla, xformla = NULL, x = NULL, data, w = NULL,
  ...
  if (se) {
    ...
    out <- QTE(
      qte = firpo.qte$qte, qte.upper = SEobj$qte.upper,
