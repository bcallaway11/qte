...
# Description: Implements the Quantile DiD estimator: the counterfactual
#   distribution is constructed by adding the untreated DiD shift to each
#   treated quantile. Supports covariates via quantile regression.
...
#' @title Quantile Difference in Differences
#' @description \code{QDiD} computes the Quantile Treatment Effect on the
#' Treated (QTET) using a Quantile Difference-in-Differences approach.
#'
#' The method can accommodate conditioning on covariates through quantile
#' regression: for each treated unit, the counterfactual outcome is computed
#' using the conditional quantile function of the untreated group, evaluated
#' at the treated unit's rank in the pre-period.
...
QDiD <- function(formla, xformla = NULL, t, tmin1, tname, data,
  ...
  if (se) {
    ...
    out <- QTE(
      qte = qdid$qte, qte.upper = SEobj$qte.upper,
