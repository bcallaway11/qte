...
#   for two-period data (CiC, compute.CiC), the ptetools-based attgt hook for
#   staggered treatment (cic_attgt), and the multi-period wrapper (cic2).
...
#'
#' @description
#' \code{compute.CiC} does the computational work for the Change in Changes
#' model of Athey and Imbens (2006). Called both for the point estimate and
...
compute.CiC <- function(qp) {
  ...
  out <- QTE(
    F.treated.t = F.treated.t, F.treated.t.cf = F.treatedcf.t,
...
# --- User-facing two-period wrapper ------------------------------------------
...
CiC <- function(formla, xformla = NULL, t, tmin1, tname, data,
  ...
  if (se) {
    ...
    out <- QTE(
      qte = cic$qte, qte.upper = SEobj$qte.upper,
...
# --- Staggered treatment (ptetools integration) ------------------------------

#' @title cic_attgt
#'
#' @description Attgt hook for the Change in Changes estimator, for use with
#'  \code{ptetools::pte2}. For each (g,t) pair, applies the CiC identification
#'  strategy to recover the ATT and optionally the full treatment effect
#'  distribution under rank invariance.
#'
#' @inheritParams ptetools::did_attgt
#'
#' @return A \code{ptetools::attgt_noif} object containing the ATT for the
#'  (g,t) pair and, via \code{extra_gt_returns}, three ECDF objects:
#'  \code{F1} (observed outcome distribution for the treated group),
#'  \code{F0} (counterfactual distribution of untreated potential outcomes),
#'  and \code{Fte} (distribution of treatment effects under rank invariance).
#'
#' @export
...
#' @title cic2
#'
#' @description Multi-period implementation of the Change in Changes approach
#'  from Athey and Imbens (2006, Econometrica) for staggered treatment adoption.
#'  Built on \code{ptetools::pte2}; returns an overall treatment effect
#'  parameter and an event study. This function is in **beta** — use caution
#'  in applied work.
#'
#' @inheritParams ptetools::pte
#' @param ret_quantile Determines which quantile is reported. Default
#'  \code{NULL} returns the ATT and an ATT event study. A value in (0,1)
#'  returns the QTT(\eqn{\tau}) and its event study. Currently implemented by
#'  evaluating \code{cic_attgt} at each (g,t) and averaging; a future version
#'  will aggregate over the full counterfactual distribution.
#' @param ret_dist If \code{TRUE}, the observed and counterfactual outcome
#'  distributions for each (g,t) pair are returned via the
#'  \code{extra_gt_returns} element of the \code{group_time_att} object.
#'
#' @references
#' Athey, Susan and Guido Imbens. ``Identification and Inference in Nonlinear
#'  Difference-in-Differences Models.'' Econometrica 74.2, pp. 431-497, 2006.
#'
#' @return A \code{pte_results} object (from \code{ptetools}) containing
#'  overall and event-study aggregations of the group-time ATTs or QTTs.
#'
#' @export
cic2 <- function(yname,
                 gname,
