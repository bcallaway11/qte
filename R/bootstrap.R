# =============================================================================
# Title: Bootstrap standard errors for QTE estimators
# Description: bootiter runs a single bootstrap iteration by resampling the
#   data and calling the supplied compute.* function. bootstrap orchestrates
#   iters calls to bootiter (via pblapply), aggregates results via computeSE,
#   and injects eachIter into the caller's environment.
...
bootiter <- function(i, qteparams, func) {
  ...
  if (qteparams$panel) {
    ids <- sample(unique(data[, idname]), length(unique(data[, idname])), replace = TRUE)
    ...
    boot.dta <- data[0, ]
    randy <- sample(seq_len(n), n, replace = TRUE)
    boot.dta <- data[randy, ]
  }
...
bootstrap <- function(qteparams, qteest, func) {
  ...
  eachIter <- pblapply(1:iters, bootiter,
    qteparams = qteparams,
    func = func, cl = cores
  )
  ## if (pl) {
