...
#' b1 <- bounds(re ~ treat,
#'   t = 1978, tmin1 = 1975, data = lalonde.psid.panel,
...
bounds <- function(formla, xformla = NULL, t, tmin1, tname, data,
  ...
  if (!(is.null(xformla))) {
    ...
    F.untreated.change.t <- stats::approxfun(pscore.data1$changey,
      pscore.data1$distvals,
      method = "constant",
      yleft = 0, yright = 1, f = 0, ties = "ordered"
    )
    class(F.untreated.change.t) <- c(
      "ecdf", "stepfun",
  ...
  ubs <- vapply(supy,
    FUN = getub, FUN.VALUE = 1,
    F.change.treated = F.untreated.change.t,
    F.treated.tmin1 = F.treated.tmin1,
    y = posvals
  )
  F.ub <- stats::approxfun(supy, ubs,
    method = "constant",
    yleft = 0, yright = 1, f = 0, ties = "ordered"
  )
  ...
  ## }
  return(BoundsObj(
    lbs = lbs, ubs = ubs, ub.quantiles = ub.quantiles,
...
summary.BoundsObj <- function(object, ...) {
  ...
  out <- list(
    lbs = bounds.obj$lbs, ubs = bounds.obj$ubs,
...
print.summary.BoundsObj <- function(x, ...) {
  ...
  ##    cat("\n")
  ## }
  bounds_print_mat(rbind(header, body), header = header)
  cat("\n")
...
#' @keywords internal
#' @noRd
bounds_print_mat <- function(m, header = NULL, digits = 2, nsmall = 2) {
  write.table(
    format(m,
      justify = "right",
...
plot.BoundsObj <- function(x, plotate = FALSE, plot0 = FALSE,
  ...
  if (is.null(ylim)) {
    ylim <- c(
      min(bounds.obj$lb.qte) - abs(median(bounds.obj$lb.qte)),
