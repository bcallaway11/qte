...
compute.panel.qtet <- function(qp) { ## function(formla, xformla=NULL, t, tmin1, tmin2,
  ...
  if (!(is.null(x))) {
    ...
    if (method == "GMM") {
      ...
      optout <- nlm(
        f = minfun, p = c(rep(0, ncol(xmat)), 0.5), datamat = datamat,
        W = solve(varmat),
        check.analyticals = T
      )
      ...
      F.untreated.change.t <- approxfun(dy.seq, F.val,
        method = "constant",
        yleft = 0, yright = 1, f = 0, ties = "ordered"
      )
      ...
      h <- 50
      pscore.reg <- nlm(
        f = log.lik, p = rep(1, ncol(xmat) - 1), y = d,
        x = x, h = h, J = J
      )
    ...
    class(F.untreated.change.tmin1) <- c(
      "ecdf", "stepfun",
  ...
  F.treated.t.cf <- approxfun(y.seq, F.treated.t.cf.val,
    method = "constant",
    yleft = 0, yright = 1, f = 0, ties = "ordered"
  )
...
MDiD <- function(formla, xformla = NULL, t, tmin1, tname, x = NULL, data,
  ...
  ## first calculate the actual estimate
  mdid <- compute.MDiD(
    formla, xformla, t, tmin1, tname, x, data,
