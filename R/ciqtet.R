# =============================================================================
# Title: Cross-sectional QTET under selection on observables (ci.qtet)
# Description: Implements Firpo (2007) propensity-score reweighting estimator
#   for the Quantile Treatment Effect on the Treated (QTET) under conditional
#   independence. Untreated quantiles are reweighted by IPW odds-ratio weights.
...
ci.qtet <- function(formla, xformla = NULL, w = NULL, data,
  ...
  if (se) {
    ...
    out <- QTE(
      qte = firpo.qtet$qte, qte.upper = SEobj$qte.upper,
