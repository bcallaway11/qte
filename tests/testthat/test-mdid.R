# =============================================================================
# Title: Fast smoke tests — mdid() staggered adoption estimator
# Description: Regression checks for mdid() covering panel ATT, repeated cross
#   sections ATT, and panel QTT. Uses biters=20 to keep runtime low.
#   Reference values from first verified run.
...
make_staggered_panel_mdid <- function(seed = 42) {
  set.seed(seed)
...
test_that("mdid panel ATT returns valid pte_emp_boot with stable estimate", {
  dat <- make_staggered_panel_mdid()
  set.seed(42)
  res <- mdid(yname = "y", gname = "group", tname = "period", idname = "id",
              data = dat, biters = 20)
...
# RCS ATT: same data treated as repeated cross sections. MDiD does not use
# unit-level panel structure, so the point estimate matches the panel version.
test_that("mdid RCS ATT returns valid pte_emp_boot with stable estimate", {
  dat <- make_staggered_panel_mdid()
  set.seed(42)
  res <- mdid(yname = "y", gname = "group", tname = "period",
...
test_that("mdid panel QTT returns valid pte_qtt with correct structure", {
  dat <- make_staggered_panel_mdid()
  set.seed(42)
  res <- mdid(yname = "y", gname = "group", tname = "period", idname = "id",
  ...
  expect_equal(res$overall$qtt[res$overall$probs == 0.5], 0.8646547, tolerance = 1e-4)
})
