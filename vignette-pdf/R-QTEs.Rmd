...
```{r, echo=FALSE, fig.cap="test caption", fig.width=15}
  library(msm)
  par(mfrow=c(1,2))
...
This is the identifying assumption made in [@fan-yu-2012; @callaway-li-2017].  This is a direct extension of the ``parallel trends'' assumption made in the Difference in Differences literature [@abadie-2005].  
Note that these types of methods only identify the QTET (not the QTE).  [@callaway-li-2017] show that this assumption only identifies the QTET when the researcher has access to three periods of panel data.

###Estimation

Estimation using the method in [@callaway-li-2017] is similar to the preceding methods.  However, the user must now additionally specify some extra parameters: (i) `t`, `tmin1`, and `tmin2` are the three time periods, and (ii) the names of the columns in `data` containing the time period and an individual id are required to be passed in as `tname` and `idname`.  `data` itself must now be a panel dataset containing individual-time period observations.  For this section the `lalonde.psid` dataset (which contains one row for each individual and with two lags of earnings available) has been converted into the `lalonde.psid.panel` (which contains three rows of observations for each individual).
...
The [@callaway-li-2017] method can be invoked with the `panel.qtet` command:

```{r panel_qtet, cache=TRUE, warning=FALSE}
...
```{r ddid2, cache=TRUE, warning=FALSE}
...
```{r qdid, cache=TRUE, warning=FALSE}
...
```{r cic, cache=TRUE, warning=FALSE}
